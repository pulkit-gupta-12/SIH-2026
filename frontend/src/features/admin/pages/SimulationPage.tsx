...
export default function SimulationPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 border-l-4 border-purple-500 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400 font-mono">Draft #{draft.id}</span>
          </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2.5 rounded-xl text-xs font-semibold bg-purple-600 hover:bg-purple-500 text-white shadow-lg transition-all cursor-pointer disabled:opacity-50"
          >
            {simulateMutation.isPending ? 'Simulating Historical Data...' : '⚡ Run Simulation Now'}
          </button>
          <button
            onClick={() => navigate(`/admin/rules/${draft.id}/publish`)}
            className="px-4 py-2.5 rounded-xl text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white shadow-md cursor-pointer"
          >
      ...
      <div className="glass-card p-4 border border-slate-800 flex items-center justify-between text-xs">
        <div>
          <span className="text-slate-400">Evaluating Condition: </span>
          <span className="font-mono text-emerald-400 font-semibold">
            {draft.proposed_condition?.type || 'required_field'} ({draft.proposed_condition?.field || 'mfg_date'})
          </span>
        </div>
        <div className="text-slate-400">
          Target Category: <strong className="text-white uppercase">{draft.category}</strong>
        </div>
      </div>
      ...
      {activeResult ? (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="glass-card p-5 border border-slate-800 space-y-1">
              ...
              <p className="text-[11px] text-slate-500">Based on historical in-force rule evaluations</p>
            </div>
        ...
        <div className="glass-card p-12 text-center space-y-4">
          ...
          <button
            onClick={() => simulateMutation.mutate()}
            disabled={simulateMutation.isPending}
            className="px-6 py-2.5 rounded-xl text-xs font-semibold bg-purple-600 hover:bg-purple-500 text-white cursor-pointer"
          >
