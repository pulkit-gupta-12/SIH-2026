...
Supports Ollama as the primary zero-cost local LLM provider.
Ensures graceful error handling when Ollama is unavailable, timed out, or uninstalled.
"""
...
class OllamaProvider(BaseLLMProvider):
    """
    Local Ollama LLM provider communicating via Ollama's HTTP REST API.
    Uses format='json' for strictly guaranteed structured JSON output.
    """
    ...
    def __init__(
        ...
        self.timeout = float(timeout or getattr(settings, "LLM_TIMEOUT_SECONDS", 10.0))

    def is_available(self) -> bool:
        """Pings Ollama's /api/version or /api/tags endpoint with a short timeout."""
        try:
            resp = requests.get(f"{self.base_url}/api/version", timeout=2.0)
    ...
    def build_prompt(
        ...
        """
        Constructs a minimal, secure system prompt and user query.
        Security: Does NOT send unnecessary package data and forbids hallucinations.
        """
        system_prompt = (
    ...
    def evaluate_semantic_rule(
        ...
        """Calls Ollama HTTP API with structured JSON response mode."""
        ...
        payload = {
            ...
            "format": "json",
            "options": {
                "temperature": 0.0,  # Deterministic low-temperature reasoning
            },
        }
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning(
                    "Ollama API returned HTTP %s for rule %s: %s",
                    resp.status_code,
                    rule_id,
                    resp.text[:150],
                )
                return {
                    ...
                    "confidence": 0.0,
                    "evidence": "",
                    "reason": f"Ollama HTTP error {resp.status_code}.",
                    "requires_human_review": True,
                }
            ...
            if not message_content:
                # Some Ollama versions return 'response' in /api/generate
            ...
            logger.warning("Ollama API timed out (%ss) for rule %s", self.timeout, rule_id)
            return {
                "rule_id": rule_id,
                "status": "REVIEW",
                "confidence": 0.0,
            ...
            logger.info("Ollama is unreachable (%s); falling back to REVIEW for rule %s", e, rule_id)
            return {
                "rule_id": rule_id,
                "status": "REVIEW",
                "confidence": 0.0,
                "evidence": "",
                "reason": "Ollama local service is unavailable.",
                "requires_human_review": True,
            }
        except (json.JSONDecodeError, TypeError) as e:
            logger.warning("Ollama returned invalid JSON for rule %s: %s", rule_id, e)
            return {
                "rule_id": rule_id,
                "status": "REVIEW",
                "confidence": 0.0,
